...
export default function Map1({ id, name, index, map }) {
    ...
    const getAllCoordinateMaps = () => {
        axios.get('http://localhost:8000/api/getAllCoordinateMaps')
            .then(response => {
                if (response) {
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            if (!unmounted) {
                axios.post('http://localhost:8000/api/allGeoMapInfo', { parentId: map.id })
                    .then(response => {
    ...
    const deleteMap = (id) => {
        if (window.confirm("Are you sure you want to delete?")) {
            ...
            axios.delete(`http://localhost:8000/api/deleteMap/${id}`)
                .then(response => {
    ...
    const addMap = (name) => {
        history(`/admin/map/${name}`)
    }

    const deleteMapName = (id) => {
        if (window.confirm("Are you sure you want to delete?")) {

            axios.delete(`http://localhost:8000/api/deleteMapName/${id}`)
                .then(response => {
