import React, { useState, useEffect } from "react";
import AlertBox from '../../../../Components/AlertComp/AlertBox';
import axios from "axios";
import { useNavigate } from "react-router-dom";
import { useSelector } from "react-redux";
import Select from "react-select";

const url = process.env.REACT_APP_API_URL;

const EditCountry = () => {
    const adminToken = localStorage.getItem('token');
    const Navigate = useNavigate();
    const selectedCountryData = useSelector((state) => state?.CountryDataChange?.payload);
    const [countryData, setCountryData] = useState({})

    useEffect(() => {
        async function getCountry() {
            try {
                const res = await axios.get(`${url}/country/get/${selectedCountryData?._id}`,
                    {
                        headers: {
                            Authorization: `${adminToken}`,
                        },
                    });
                setCountryData(res?.data?.country || {});
            } catch (error) {
                console.log(error)
            }
        }
        getCountry();
    }, [selectedCountryData]);


    const [countryName, setCountryName] = useState(selectedCountryData?.name || '');
    const [statusMessage, setStatusMessage] = useState('');
    const [countryAddStatus, setCountryAddStatus] = useState('');

    const handleSubmit = async (e) => {
        e.preventDefault();

        if (countryName !== "") {
            const formData = { "name": countryName }
            try {
                const response = await axios.put(
                    `${url}/country/update/${selectedCountryData?._id}`,
                    formData,
                    {
                        headers: {
                            Authorization: `${adminToken}`,
                        },
                    }
                );

                if (response.data.type === "success") {
                    setCountryAddStatus(response.data.type);
                    setStatusMessage(response.data.message);

                    setTimeout(() => {
                        Navigate("/admin/showCountry");
                    }, 900);
                } else {
                    setCountryAddStatus(response.data.type);
                    setStatusMessage(response.data.message);
                }
            } catch (error) {
                setCountryAddStatus("error");
                setStatusMessage("Country not Update !");
            }
        }
    };

    useEffect(() => {
        const timer = setTimeout(() => {
            setCountryAddStatus("");
            setStatusMessage("");
        }, 1500);

        return () => clearTimeout(timer);
    }, [countryAddStatus, statusMessage]);


    return (
        <>
            <div className="main-content dark">
                <div className="page-content">
                    <div className="container-fluid">
                        <div className="row">
                            <div className="col-12">
                                <div className="page-title-box d-flex align-items-center justify-content-between">
                                    <h4 className="mb-0">Edit Country</h4>
                                </div>
                            </div>
                        </div>
                        <div className="row">
                            <div className="col-12">
                                <div className="card">
                                    <div className="card-body">
                                        <form onSubmit={handleSubmit}>
                                            <div className="mb-3 row">
                                                <label
                                                    htmlFor="example-text-input"
                                                    className="col-md-2 col-form-label"
                                                >
                                                    Country Name:
                                                </label>
                                                <div className="col-md-10">
                                                    <input
                                                        required
                                                        className="form-control"
                                                        type="text"
                                                        id="example-text-input"
                                                        value={countryName}
                                                        onChange={(e) => {
                                                            setCountryName(e.target.value);
                                                        }}
                                                    />
                                                </div>
                                            </div>
                                            <div className="mb-3 row">
                                                <div className="col-md-10 offset-md-2">
                                                    <div className="row mb-10">
                                                        <div className="col ms-auto">
                                                            <div className="d-flex flex-reverse flex-wrap gap-2">
                                                                <a
                                                                    className="btn btn-danger"
                                                                    onClick={() => Navigate("/admin/showCountry")}
                                                                >
                                                                    {" "}
                                                                    <i className="fas fa-window-close"></i> Cancel{" "}
                                                                </a>
                                                                <button className="btn btn-success" type="submit">
                                                                    {" "}
                                                                    <i className="fas fa-save"></i> Save{" "}
                                                                </button>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <AlertBox status={countryAddStatus} statusMessage={statusMessage} />
                                                </div>
                                            </div>
                                        </form>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </>
    );
};

export default EditCountry;
