...
const ShowOrderDetails = () => {
    ...
    useEffect(() => {
        async function getCompanyData() {
            try {
                let response = await axios.get(`${url}/data/get`,
                    {
                        headers: {
                            Authorization: `${adminToken}`,
                        },
                ...
                if (orderDetails?.ShippingType === "1") {
                    setCompanyData(response?.data?.CourierData || [])
                }
                else if (orderDetails?.ShippingType === "2") {
                    setCompanyData(response?.data?.TransportData || [])
                }
    ...
    //     await setSelectedOrderProductData(product)
    ...
    const handleOrderProductDelete = async (product) => {
        try {
            const orderProductId = product._id;
            ...
            if (result.isConfirmed) {
                const response = await axios.delete(
                    `${url}/order/modify/product/delete/${selectOrderId}/${orderProductId}`,
                    {
                        headers: {
                            Authorization: `${adminToken}`,
                        },
                ...
                <div className="page-content">
                    <div className="container-fluid">
                        <div className='row'>
                            ...
                            <div className="col-2 " >
                                <PDFDownloadLink document={<OrderInvoicePDF orderDetails={orderDetails} productDetails={productDetails} />} fileName={`${orderDetails?.User_Name}_${new Date().toISOString().split('T')[0]}_packing_slip.pdf`}>
                            ...
                            <div className="col-2 ">
                                <PDFDownloadLink document={<OrderLabelInvoicePDF orderDetails={orderDetails} productDetails={productDetails} addressDetails={addressDetails} />} fileName={`${orderDetails?.User_Name}_${new Date().toISOString().split('T')[0]}_order_label_invoice.pdf`}>
                                    {({ blob, url, loading, error }) =>
                                        ...
                                        <form >
                                            ...
                                            <div className='mt-2' >
                                                ...
                                                <div className="mb-3 row">
                                                    ...
                                                    <div className="col-md-5 mt-1">
                                                        <input type="text" name="name" id="name" value={new Date(orderDetails?.createdAt).toLocaleDateString('en-IN')} className="form-control disableColor" disabled />
                                                    </div>
                                                    <div className="col-md-5 mt-1">
                                                        <input type="text" name="name" id="name" value={new Date(orderDetails?.createdAt).toLocaleTimeString('en-IN', { hour12: true })} className="form-control disableColor" disabled />
                                                    </div>
                                                ...
                                                <div className="mb-3 row">
                                                    ...
                                                    {orderDetails?.ShippingType !== "0" &&
                                                        <>
                                                            ...
                                                            <div className="col-md-10 mt-1">
                                                                <select name="o_type" id="o_type" style={{ width: "100%", height: "100%" }} className="select2" onChange={(e) => setCompanyName(e.target.value)} >
                                                                    <option value="" selected>Select Company</option>
                                                                    {companyData?.map((company, index) => {
                                                                        if (company?.Data_Name === orderDetails?.Company) {
                                                                            return <option key={index} value={company?.Data_Name} selected>{company?.Data_Name}</option>
                                                                        }
                                                                        else {
                                                                            return <option key={index} value={company?.Data_Name}>{company?.Data_Name}</option>
                                                                        }
                                                                    })}
                                                                </select>
                                                            </div>
                                                    ...
                                                    <div className="col-md-10 mt-1">
                                                        <input type="text" name="name" id="name" value={Math.ceil(orderDetails?.total_weight * 100) / 100 + " Kg"} className="form-control disableColor" disabled />
                                                    </div>
                                                    <label htmlFor="example-text-input"
                                                        className="col-md-2 col-form-label">
                                                        Track_Id :-
                                            ...
                                            <div className="row mb-10">
                                                <div className="col ms-auto">
                                                    <div className="d-flex flex-reverse flex-wrap gap-2">
                                                        <a className="btn btn-danger" onClick={() => navigate('/admin/showOrders')}>
                                                            {" "}
