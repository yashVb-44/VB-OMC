...
route.post('/create/byAdmin', checkAdminRole, async (req, res) => {
    ...
    try {
        ...
        if (admin) {
            return res.status(201).json({ type: 'error', message: 'Sub-admin with this username already exist!' });
        }
...
route.get('/get', checkAdminRole, async (req, res) => {
    try {
        ...
        const result = subAdmins.map(admin => {
            ...
            let role = ''
            if (admin.role === "role_1") {
                role = 'Marketing';
            } else if (admin.role === "role_2") {
                role = 'Packaging';
            } else if (admin.role === "role_3") {
        ...
        res.status(200).json({ type: 'success', message: 'Sub-Admin find successfully', subAdmins: result });
    } catch (error) {
        res.send(error)
        console.log(error)
    }
...
route.put('/edit/byAdmin/:id', checkAdminRole, async (req, res) => {
    const id = req.params.id;
